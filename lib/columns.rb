class Columns
  ...
  def self.create_table_statements(conf)
    ...
    # Execute Query to get list of columns and table information
    ...
    # Build a datastructure which stores schema -> tables -> columns      
    all_tables = []
    copy_selects = []
    ...
    # Iterate over datastructure and generate sql strings
    #   A couple SQL strings with different purpose are created in parallel.
    #     - The table_defintion tables are foreign server tables accessible through the FDW.
    #     - The mirror_table_definition tables are native postgres tables which data is optionally copied into if parameters provided.
    #  
    error_messages = []
